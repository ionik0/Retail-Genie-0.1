...
import { orchestratorAPI, recommenderAPI, deepseekAPI } from '../services/api';
...
const Chatbot = ({ customer, onNavigate }) => {
  ...
  useEffect(() => {
    // Initial greeting from ABFRL Sales Agent
    setTimeout(() => {
      addBotMessage(`Hello ${customer.name}! 👋 Welcome to ABFRL. I'm the **ABFRL Sales Agent**, powered by Deepseek AI. How can I help you find the perfect outfit today?`);
      setTimeout(() => {
        addBotMessage("What are you looking for? You can tell me:\n• Wear type: Party wear, Wedding wear, Street wear, Office wear, Casual, Traditional\n• Specific item: Kurtas, Shirts, Jeans, Shoes, Blazers, etc.\n• Or just describe what you need!");
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const response = await deepseekAPI.sendMessage(userText);
      
      // Handle response from Deepseek
      if (response && response.response) {
        addBotMessage(response.response, 'text');
      } else if (response && response.message) {
        addBotMessage(response.message, 'text');
      } else {
        addBotMessage('I understand. How else can I help you?', 'text');
      }
      
      setChatStage('conversation');
    } catch (error) {
      console.error('Error sending message to ABFRL Sales Agent:', error);
      
      // Fallback to local recommendation if Deepseek API fails
      ...
      if (recommendations.length > 0) {
        ...
        addBotMessage(
          `I'm having trouble connecting to the ABFRL Sales Agent right now. Let me show you some options:\n\nYou can ask me about:\n• Party wear, Wedding wear, Office wear\n• Specific items: Shirts, Jeans, Kurtas, Shoes\n• Your budget or style preference`,
          'text'
        );
      }
    } finally {
      ...
      <div className="bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 text-white p-4 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">ABFRL Sales Agent (Deepseek AI)</h2>
